...
- **内嵌阿里巴巴普惠体（精简版）**：键盘与设置界面统一使用内嵌字体，离线 / WinPE 环境无需系统字体即可获得一致外观。
...
从主界面“菜单”按钮或托盘右键“设置”打开，左侧 Tab（常规 / 主题 / 关于，关于在最底），**修改即时生效**；配置保存在 exe 同目录 `HKeyboard.ini`（首次启动自动生成、按需写入），窗口大小与主题选择也会自动记录并在下次启动恢复：

- **常规**：自动呼出开关；关闭按钮（×）行为——直接退出程序 / 隐藏到系统托盘，可勾选“记住我的选择”持久化（重启后仍生效）；按 × 时弹出关闭方式提示，已记住则直接按所记方式执行。
- **主题**：跟随系统 / 深色主题 / 浅色主题；高亮按钮跟随壁纸强调色（对应 `-theme:system` / `-dark` / `-light` / `-wallpaper`）。
- **关于**：版本与作者信息。
