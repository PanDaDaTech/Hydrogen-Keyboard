...
- **智能焦点感应自动呼出（默认开启）**：深入透视 Caret 闪烁光标，精准识别 QQ、微信、Chrome、Notepad、Office 等输入框，点击秒级自动滑出，离焦自动收回。可在主界面菜单中勾选“自动呼出”启停，也可用 `-auto` / `-noauto` 命令行参数指定。
...
- **中英文一键切换**：点击 `Shift` 键第 1 次锁定并高亮（用于 `Shift+组合键`），第 2 次点击切换当前输入法的中英文状态；使用 `Shift+组合键` 后状态自动复位，可再次正常使用。
- **Win 键（参考大写键）**：第一次点击 `Win` 键锁定并高亮，随后点击其他按键可发送 `Win+按键`；第二次点击轻按一次 `Win` 键（打开开始菜单）并解除锁定、取消高亮；之后每次点击都轻按 `Win` 键，开始菜单随点击开 / 关，不会失步。
