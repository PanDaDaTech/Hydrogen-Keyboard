...
- **QQ 风格桌面边缘隐藏**：将键盘拖到屏幕左 / 右 / 上边缘后，鼠标离开会自动收起为一条细边，悬停或点击即可滑出，节省屏幕空间。
...
- **Win 键开关开始菜单**：单击 `Win` 键打开开始菜单并锁定（高亮），随后点击其他按键可发送 `Win+按键`；再次单击 `Win` 关闭开始菜单并解除锁定。连续点击可稳定交替开 / 关，不会像旧版那样第 3、4 次点不开。
